...
func NewCreateSessionUseCase(sessionRepo session.Repository) *CreateSessionUseCase {
...
func (uc *CreateSessionUseCase) Execute(ctx context.Context, req CreateSessionRequest) (*CreateSessionResponse, error) {
	...
	if exists {
		return nil, session.ErrSessionAlreadyExists(req.Name)
	}
...
func (uc *CreateSessionUseCase) validateRequest(req CreateSessionRequest) error {
	...
	if name == "" {
		return session.ErrInvalidSessionName("name cannot be empty")
	}

	if len(name) > 255 {
		return session.ErrInvalidSessionName("name cannot exceed 255 characters")
	}
