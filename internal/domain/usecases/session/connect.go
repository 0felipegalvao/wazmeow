...
func NewConnectSessionUseCase(sessionRepo session.Repository, whatsappClient whatsapp.ClientManager) *ConnectSessionUseCase {
...
func (uc *ConnectSessionUseCase) Execute(ctx context.Context, req ConnectSessionRequest) (*ConnectSessionResponse, error) {
	...
	if !sess.CanConnect() {
		return nil, session.ErrCannotConnect(sessionID, sess.Status())
	}
